...
public class QnAPagenation {
    ...
    public static QnASelectCriteria QnAGetSelectCriteria(int pageNo, int totalCount, int limit, int buttonAmount  ) {

        return QnAGetSelectCriteria(pageNo, totalCount, limit, buttonAmount, null, null);
    }


    /* 검색어가 존재하는 경우 검색 조건으로 select 후 페이징 처리를 하기 위한 용도 */
    public static QnASelectCriteria QnAGetSelectCriteria(int pageNo, int totalCount, int limit, int buttonAmount, String nation3, String searchValue) {
...
//		startPage = (((int)((double) pageNo / buttonAmount + 0.9)) - 1) * buttonAmount + 1;
        ...
        QnASelectCriteria QnAselectCriteria = new QnASelectCriteria(pageNo, totalCount, limit, buttonAmount ,maxPage, startPage, endPage, startRow, endRow, nation3, searchValue);

        return QnAselectCriteria;
    }
